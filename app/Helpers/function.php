...
function setParams($key,$value){
    ...
    $last_point=0;
    if (strpos($url,'?')){
        do if (strpos($url,$key,$last_point)&&strlen($key)==
            (strpos($url,'=',strpos($url,$key,$last_point))-strpos($url,$key,$last_point)))
           return $url = substr($url,0,strpos($url,$key,$last_point)+strlen($key)+1).
                   $value.(strpos($url,'&',strpos($url,$key,$last_point)
                   +strlen(strpos($url,$key,$last_point)))
